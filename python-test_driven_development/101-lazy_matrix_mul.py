...
def lazy_matrix_mul(m_a, m_b):
    ...
    if not any(m_a):
        raise ValueError("shapes (1,0) and (2,2) not"
                         " aligned: 0 (dim 1) != 2 (dim 0)")
    if not any(m_b):
        raise ValueError("shapes (2,2) and (1,0) not "
                         "aligned: 2 (dim 1) != 1 (dim 0)")
    ...
    if len(m_a[0]) != len(m_b):
        raise ValueError("shapes (2,3) and (2,2) not "
                         "aligned: 3 (dim 1) != 2 (dim 0)")
